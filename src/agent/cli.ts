...
async function run(): Promise<void> {
  ...
  if (command === "send") {
    ...
    const result = await requestJson("/v1/telegram/send", {
      method: "POST",
      headers: { "content-type": "application/json" },
      body: JSON.stringify({
  ...
  if (command === "delete") {
    ...
    const result = await requestJson("/v1/telegram/delete", {
      method: "POST",
      headers: { "content-type": "application/json" },
  ...
  if (command === "upload") {
    ...
    const result = await requestJson("/v1/telegram/upload", { method: "POST", body: form });
    console.log(JSON.stringify(result, null, 2));
    return;
  }
  if (command === "upload-album") {
    ...
    const result = await requestJson("/v1/telegram/upload-album", {
      method: "POST",
      body: form,
    });
    console.log(JSON.stringify(result, null, 2));
